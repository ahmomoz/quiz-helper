...
1.  **Refactor UI Components**
    ...
    *   *Validation*: Check existing usages (mostly in `QuizPage`) and temporarily fix them or do this in parallel with step 3. (Since `QuizPage` is being rewritten, we can break it momentarily or update it in step 3).
...
2.  **Implement `useQuiz` Hook**
    ...
    *   *Validation*: Unit test the hook (optional but recommended) or verify via manual testing after integration.
...
3.  **Decompose `QuizPage` Components**
    ...
    *   *Validation*: Ensure each component renders with mock data if needed.
...
4.  **Integrate and Finalize `QuizPage`**
    ...
    *   *Validation*: Run the app and verify the entire quiz flow works identical to before, but with better code structure.
